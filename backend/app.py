...
def login():
    ...
    try:
        ...
        cursor = conn.cursor(cursor_factory=RealDictCursor)
        cursor.execute(
            "SELECT id, nome, email, senha_hash FROM Usuarios WHERE email = %s", (email,)
        )
        row = cursor.fetchone()
        conn.close()
...
def api_calendario_info():
    ...
    try:
        ...
        cursor.execute(
            ...
            WHERE usuario_id = %s
            AND (
                TO_CHAR(inicio, 'YYYY-MM') <= %s
                AND (
                    fim IS NULL
                    OR TO_CHAR(fim, 'YYYY-MM') >= %s
                )
            )
            """,
            (
...
def api_sintomas_dia():
    ...
    try:
        ...
        cursor.execute(
            ...
            WHERE usuario_id = %s
            AND (
                (fim IS NULL AND TO_CHAR(inicio, 'YYYY-MM-DD') = %s)
                OR
                (
                    fim IS NOT NULL
...
def api_sintoma_detail(sintoma_id):
    if request.method == "DELETE":
        try:
            ...
            cursor = conn.cursor(cursor_factory=RealDictCursor)
            cursor.execute(
                "DELETE FROM Sintomas WHERE id=%s AND usuario_id=%s",
    ...
    try:
        ...
        cursor.execute(
            """UPDATE Sintomas SET tipo=%s, subtipo=%s, descricao=%s,
